/*
 * HIP test for GLOBAL_LOAD_SBYTE_D16_HI (Inst_FLAT, GLOBAL segment).
 *
 * GLOBAL_LOAD_SBYTE_D16_HI loads one byte from memory, SIGN-extends it to
 * 16 bits, and writes it into the HIGH 16 bits of the destination VGPR
 * while PRESERVING the low 16 bits:
 *
 *   VDST[31:16] = sign_extend(MEM[addr][7:0])   (8-bit -> 16-bit)
 *   VDST[15:0]  = VDST[15:0]                      (unchanged)
 *
 * The gem5 implementation reads memory as VecElemI8 and casts to VecElemI16
 * (sign extension), then replaceBits(vdst[lane], 31, 16, tmp) after a
 * vdst.read() that preserves the lower half.  This test exercises bytes
 * with the sign bit set and verifies the low half (sentinel 0xBEEF) is
 * untouched.
 *
 ...
 *
 * Run in gem5:
 *   gem5.opt config.py -- ./global_load_sbyte_d16_hi
 */
...
/*
 * kernel_load_sbyte_d16_hi
 *
 * Each thread sign-extends in[i] (a single byte) into the HIGH half of a
 * VGPR seeded with the sentinel.  "+v" keeps the preserved low half.
 */
__global__ void
kernel_load_sbyte_d16_hi(const int8_t *in, uint32_t *out, int n)
{
    ...
    __asm__ volatile(
        "global_load_sbyte_d16_hi %0, %1, off\n\t"
        "s_waitcnt vmcnt(0)\n\t"
        : "+v"(vd)
        : "v"(addr)
        : "memory");
...
main(void)
{
    ...
    hipLaunchKernelGGL(kernel_load_sbyte_d16_hi,
    ...
    for (int i = 0; i < N; i++) {
        ...
        uint32_t hi       = (uint32_t)(uint16_t)sext;
